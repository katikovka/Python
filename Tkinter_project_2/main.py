...
def question_Anna():
    ...
    text.configure(text="""
                            ...
                            с черной сумочкой для ноутбука.
                            Его зовут Альберт...Андреевич.
                            Пара началась с того, что два человека защищают доклад. 
                            Первая выходит Аня. После того, как она закончила, Альберт Андреевич спрашивает, 
                            есть ли у кого-то вопросы.
                            """)
    text_question.configure(text="Задать вопрос Ане?")
    b.configure(command=partial(question_ignat))

    text_question.pack
    var1 = Button(menu, text="Да", command=lambda i=None:
    (
        text.configure(text=text.cget("text") + "Аня как обычно придумала, что ответить."), var1.destroy(),
        var2.destroy()))
    var2 = Button(menu, text="Нет", command=lambda i=None:
    (text.configure(text=text.cget("text") + "Вы не задави вопрос, Альбер думает, что Вы глупый"), var1.destroy(),
     var2.destroy()))
    var1.pack()
    var2.pack()
...
def question_ignat():
    text.configure(text="""
                        ...
                        Задать вопрос Игнату?
                    """)
    text_question.configure(text="Задать вопрос Игнату?")
    var1 = Button(menu, text="Да", command=lambda:
    (
        text.configure(
            text=text.cget("text") + "Игнат не смог ответить на этот вопрос.\nОн обиделся, вы больше не друзья"),
        var1.destroy(), var2.destroy(), add_steps("question", 1), b.configure(command=partial(hall))))
    var2 = Button(menu, text="Нет", command=lambda:
    (text.configure(text=text.cget("text") + "Вы не задави вопрос, Альбер думает, что Вы глупый"),
     var1.destroy(), var2.destroy(), add_steps("question", 0), b.configure(command=partial(hall))))
    text_question.pack
    var1.pack()
    var2.pack()
...
def lose():
    ...
    (text.configure(text=text.cget("text") + "\nВы пошли налево..."),
     ...
     (text.configure(text=text.cget("text") + "\nВы пошли налево..."),
      ...
      (text.configure(text=text.cget("text") + "\nВы пошли направо..."),
       var2.pack_forget(), var1.pack_forget(),
    ...
    (text.configure(text=text.cget("text") + "\nВы пошли вперёд..."), var3.pack_forget()))))
...
def dining():
    ...
    def cash_ost(cash_lbl, rad1):
        ...
        if (MONEY - int(result)) >= 0:
            buy.configure(text="Забрать", command=lambda: (buy.destroy(),
                                                           text_question.configure(
                                                               text="Еда куплена. Теперь можно и покушать"),
                                                           [i.destroy() for i in rad_list], new_friend(),
                                                           cash_lbl.destroy(), price.destroy(), amount_label.destroy()))
            ...
            buy.configure(
                command=lambda: (cash_lbl.configure(text=f"На вашем счету: {MONEY}"), cash_ost(cash_lbl, rad1)))
    ...
    if steps["chose"] == 1:
        if steps["question"] == 1:
            if steps["lose"] == 1:
                text.configure(text=f"""Наконец-то Вы дошли до столовой! Это было сложно.
                ...
                text.configure(text=f"""
                    ...
                    *желудок грустно урчит*
                    """)
                b.configure(command=lambda: ([i.destroy() for i in rad_list], new_friend(steps),))
        else:
            text.configure(text=f"""
        ...
        text.configure(text=f"""
            ...
            Так как вы сели с Мишей и подружились с ним во время первой и второй пары,
            он поможет вам без проишествий дойти до столовой.
        ...
        if steps.get('lose') == 1:
            text.configure(text=f"""
                Едите в одиночестве, не заблудились.
                """)
        else:
            text.configure(text=f"""
                Заблудильсь, не успели поесть и опоздали на пару.
                """)
            b.configure(command=lambda: (lesson3()))
    ...
    cash_lbl = Label(menu, text=f"На вашем счету: {MONEY}")
...
def test():
    title.configure(text="Пара Торшина")
    text.configure(text="Торшин даёт тест...")
    ...
    if steps["chose"] == 1:
        if steps["question"] == 1:
            ...
            text.configure(text="""
                    ...
                    Он любехно позвал Вас поехать вместе.
                    """)
...
def cost():
    ...
    if (CASH - MONEY) >= 0:
        ...
        cash_lbl = Label(menu, text=f"Недостаточно средств")
    ...
    if steps["chose"] == 1:
        text.configure(text=
                       """У меня есть для Вас две новости!
                            ...
                            """)
        b.configure(text="Начать заново", command=lambda: (title.configure(text=""), person()))
    elif steps["chose"] == 2:
...
def misha(user_name):
    ...
    text.configure(text=
                   f"""
                Вы проходите на первую парту к Мише. Первая пара Кулакова.
                Он заходит в аудиторию. Как обычно достаёт свой термо-стакан с кофе и черную 
                бутылку с неизвестной никому жидкостью.
                Кулаков: 'Всем привет, сегодня у нас с вами очередной нудный понедельник.'
                Далее он объясняет новую тему.
                Все полтора часа вы болтали с Мишей. Вы хорошо познакомились и даже немного
                подружились.
                Начинается перерыв.
                """, font=("Arial Bold", 20), justify=LEFT)
...
def lesson3():
    ...
    if steps["chose"] == 1:
        ...
        if steps["lose"] == 1:
            ...
            text.configure(text="""
                                Пара уже началась. С трудом Вы нашли обратный путь.
...
def free_desk(user_name):
    ...
    *Далее мы наблюдаем как какой-то дядя с хвостиком и сережкой в ухе раскладывает 
    свои 56798274892 слайдов.*""",
                   font=("Arial Bold", 20), justify=LEFT)
    b.configure(text="Ёмае! А он крут!", font=("Arial Bold", 20),
                width=15, height=2, command=partial(sleeping))
...
def sleeping():
    ...
    rad2.pack()
    b.configure(text="Продолжить", font=("Arial Bold", 20), width=17, height=2,
                command=partial(sleep, rad1, rad2, radioValue))
...
def sleep(rad1, rad2, radioValue):
    ...
    if radioValue.get() == 0:
        text.configure(text=
                       f"""
        Поздравляю! 
        Теперь вы полтора часа будете слушать кучу баек вперемешку с непонятными терминами!""")
        b.configure(text="Продолжить", font=("Arial Bold", 20), width=17, height=2,
                    command=partial(secondLesson))
...
def secondLesson():
    ...
    text.configure(text=
                   f"""
            Итак, вы пережили первую пару, поздрвляю!
            Начинается вторая пара Альберта Андреевича - Телкоммуникационные сети и технологии
            """)
    b.configure(text="Продолжить", font=("Arial Bold", 20), width=17, height=2,
                command=partial(question_Anna))
